...
```
##Abstract
The main objective of this project is to explore and make sense of the massive quantities of data available on amazon reviews.  Are there any patterns present within individual reviews that we can use to draw conclusions about the rating of that review? In order to answer this question, we focused on looking for relationships between the score of a single review (from 1-5) and the rest of the variables in our dataset.  Our final deliverable is a model that predicts what the rating of a review will be based on the rest of the components of review, such as its length and how many people voted it as “helpful”.  We also created a latent dirichlet model to determine if there were any hidden topics within the data that could help us predict rating.

##Introduction
How can we predict the rating of an amazon review? Although each review has only a few fields, there is plenty of insight to be gained when you look deep enough.  Therefore, in our analysis, we explored both the easily accessible fields in our data, such as review wordcount and number of helpful votes, as well as the latent ones, such as review subtopic, which we accessed through text analysis.
	First we looked at the fields which were easily accessed in our dataset, such as review length and number of helpful votes.  However, after an extensive EDA and basic multinomial logistic regression, we weren’t able to produce a model that had sufficient predictive power.  Therefore, we turned to the review text in order to predict rating.
First, we broke down our text analysis by created a latent dirichlet model.  We expected our LDA, a common tool for identifying subtopics in texts, to determine the overall topic of the review.  For example, if the topic was defined by words such as “poor” or “disappointed,” then the rating would be expected to be low, and vice versa.  After removing common contractions and words that would be otherwise unhelpful to our final output, we created our LDA to find those hidden topics within the review text.  However, instead of identifying positive/negative sentiments, our model seemed to simply identify the product categories that we drew our data from.  For example, we were very convinced that the category containing words like “album”, “song”, “sound”, and “track” simply confirmed that we pulled part of our review data from the Digital Music category.  
...
ggplot(df2_sampled,aes(x=Rating,fill=reviewLengthFactor)) +
  geom_bar(stat="count") +
  ggtitle("Rating Distriubtion based on Review Length")
```
...
```{r}
# Tokenize review text, create word stems, and remove stop words
...
Our results point towards a relationship between review text data and review rating since our model when including information about text data performed significantly better than our model that did not contain any text data. We ended up with an MSE of .65 using our model on out-of-sample data. This result is ok, but not good enough to be of much use. 
However, there is much more that we could have done to improve our model’s performance. We could have identified common words that don’t have much meaning and removed them, created n-grams of the data, performed transformations on the data, performed some sort of sentiment analysis, combined text and non-text features, or experimented with different types of models. Our model barely scratches the surface of the text mining and feature engineering that we could have done with the text in the review data. We decided to focus our project on managing the large amount of data that we had in hadoop instead of experimenting with models and feature engineering. 
Moreover, we only performed our analysis on a small portion of the data that we had, sampling from subset of review categories. We did not manage to get spark to work efficiently and primarily used our cluster to simply hold the data. Our results should not be used for any serious purposes, as our model is not representative of the entire amazon review dataset and our model does not take into account many important text features that can be extracted. 
