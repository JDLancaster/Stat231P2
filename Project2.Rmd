...
df2<-df2 %>% 
  mutate(helpful=test[c(T,F)],not_helpful=test[c(F,T)]-test[c(T,F)])
```
...
#Helpful
a<-ggplot(df2,aes(x=helpful))+geom_bar(stat="count")+scale_x_continuous(limits = c(-1,5))
b<-ggplot(df2,aes(x=not_helpful))+geom_bar(stat="count")+scale_x_continuous(limits = c(-1,5))

ggplot(df2, aes(helpful)) + 
  geom_bar(aes(x = helpful, stat="count")) + 
  geom_bar(aes(x = not_helpful, stat="count")) +
...
ggplot(helpful_frame, aes(, fill = veg)) + geom_density(alpha = 0.2)



a+b
a+b+scale_x_continuous(limits = c(-1, 10),breaks=c(0,2,4,6,8,10))+geom_bar(aes(fill = drv), position = position_stack(reverse = TRUE)) +
coord_flip() +
theme(legend.position = "top")


#Rating
ggplot(df2,aes(x=Rating))+geom_bar(stat="count",aes(fill=as.factor(Rating)))+geom_text(stat='bin',aes(label=..count..),vjust=-1,binwidth=1)+ggtitle("Ratings Distribution")+scale_fill_hue(c=500, l=100)+theme(legend.position="none")
#ReviewLengthFactor
ggplot(df2,aes(x=reviewLength))+geom_bar(stat="count")
