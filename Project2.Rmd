...
musical_instruments <- jsonlite::stream_in(file("Project Data/reviews_Musical_Instruments_5.json"))
...
df<-rbind(amazon_instant,apps_for_android,automotive,musical)
```
...
musical$helpful<-toString(musical$helpful)
str(musical$helpful)
musical$helpful[2]
ay<-musical$helpful[2]
ay
table(musical$helpful)
str(musical$helpfulp)
readr::parse_number(sub(" .*", "", ay))
as.numeric(gsub("([0-9]+).*$", "\\1", sub(" .*", "", ay)))
```
...
```{r}
df<-select(df,reviewerID,asin, reviewText, overall, summary, reviewTime)
```
...
#Helpful
ggplot(df2,aes(x=helpful))+scale_x_continuous(limits = c(-1, 10),breaks=c(0,2,4,6,8,10))+geom_bar()
