...
df2 <- deduped%>% 
  ...
  mutate(helpful_final = dplyr::combine(helpful)[c(T,F)],
         not_helpful_final = 
           dplyr::combine(helpful)[c(F,T)] - 
           dplyr::combine(helpful)[c(T,F)],
         perc_helpful_final =ifelse(helpful_final==0 , 0, 
         helpful_final / (helpful_final + not_helpful_final)), 
         Rating_Scaled=Rating/5,
         reviewLengthFactor=ifelse(
           reviewLength<200,"Short",
           ifelse(reviewLength<600,"Medium","Long"))) %>% 
  select(-helpful)
colnames(df2)
...
it_train <- itoken(df_features$reviewText, 
...
NFOLDS = 4
t1 = Sys.time()
glmnet_classifier = cv.glmnet(x = dtm_train, y = df_features[['Rating']], 
                              family = 'multinomial', 
                              # L1 penalty
                              alpha = 1,
