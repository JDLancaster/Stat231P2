...
```{r}
##Tier 1: Rating
ggplot(df2a,aes(x=Rating))+geom_bar(stat="count",aes(fill=as.factor(Rating)))+geom_text(stat='bin',aes(label=..count..),vjust=-1,binwidth=1)+ggtitle("Ratings Distribution")+theme(legend.position="none")
#Ratings vs Helpful
ggplot(df2a,aes(x=Rating,y=perc_helpful_final,group=Rating))+geom_boxplot()+scale_y_continuous(limits = c(0.00001,1))+ggtitle("Percent of votes as 'Helpful' vs Rating")+ylab("% of Votes as 'Helpful'")
#Ratings vs ReviewLength
ggplot(df2a,aes(x=Rating,fill=reviewLengthFactor))+geom_bar(stat="count")+ggtitle("Rating Distriubtion based on Review Length")


##Tier 1: ReviewLength
ggplot(df2a,aes(x=reviewLength,fill="red"))+geom_density()+scale_x_continuous(limits = c(0,1000))+theme_classic()+theme(legend.position="none")+ggtitle("Distribution of ReviewLength")#Values more than 1000 continued the trend
#ReviewLength vs Helpful
ggplot(filter(df2a,helpful_final<100), aes(x=reviewLength, y=helpful_final))+geom_point()+stat_smooth()+ylab("Helpful Votes")+ggtitle("ReviewLength vs # of Helpful Votes")
```
...
ggplot(filter(prolific_users,popularity<1000 & reviews<45),aes(x=reviews,popularity))+geom_point()+stat_smooth()+ggtitle("Mean Review Length (per user) vs num Reviews (per user)")
...
#NumReviews vs Helpful
ggplot(filter(prolific_products,reviews<45),aes(x=reviews,y=avg_popularity))+geom_point()+stat_smooth()+ggtitle("Mean Review Length (per product) vs num Reviews (per product)")
```
...
my_title<-"Categories grouped on: Amazon Instant Video, Automotive, Digital Music, Musical Instruments, Office Products, Patio/Lawn, Pets, and Tools"
...
ap_top_terms %>%
  ...
  coord_flip()+ggtitle(wrapper(my_title, width = 20))
